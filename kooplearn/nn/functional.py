...
def covariance(X: torch.Tensor, Y: Optional[torch.Tensor] = None, center: bool = True):
    ...
    Returns:
        torch.Tensor: Covariance matrix of shape ``(features, features)``. If ``Y is not None`` computes the
...
def vectorized_cross_cov(X_contexts: torch.Tensor,
    ...
    Returns:
        CCov: (time_horizon, time_horizon, state_dim, state_dim) Tensor containing all the Cross-Covariance
         ...
         # TODO: this method creates a large memory footprint to gain computational speed.
    """
    ...
    if run_checks:  # Sanity checks. These should be true by construction
        for t in range(min(pred_horizon, cov_window_size)):
            ...
            CovXY_true = covariance(X=X_contexts[:, t, :], Y=Y_contexts[:, t+1, :], center=False)
            assert torch.allclose(CovXY[t, t+1], CovXY_true, rtol=1e-5, atol=1e-5), \
                f"Max error {torch.max(torch.abs(CovXY[t, t+1] - CovXY_true))}"
